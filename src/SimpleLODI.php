...
class SimpleLODI {
	...
	function getHTML($data, $url) {
		...
		$loader = new \Twig\Loader\FilesystemLoader($this->root_dir.'templates');
		$twig = new \Twig\Environment($loader);

		$function = new \Twig\TwigFunction('bnode_info', function ($val,$_data) {
		...
		$function2 = new \Twig\TwigFunction('shorten', function ($uri) {
			$short = \EasyRdf\RdfNamespace::shorten($uri);
			return $short?$short:$uri;
		});
		$twig->addFunction($function2);

		$function3 = new \Twig\TwigFunction('getlatlng', function($_rdfs) {
